...
* 회원가입과 로그인 기능을 만들고 그에 따른 Member 엔티티를 추가했지만 Post와 Comment 엔티티와 연관관계 설정은 아직 하지 않음

* HomeController, MemberController, LoginController 추가함

* 세션을 이용해서 로그인후 홈화면으로 가면 로그인한 사용자의 이름이 표시되게 추가하고, 조회수 기능도 쿠키대신 세션을 이용함

## 하면서 배운 사실

### 1.세션

* 쿠키는 그대로 노출되고 다른사람이 임의로 변경가능해서 조작할 수 있다는 취약점이 있음

* 이를 위해서 세션은 Map<JSESSIONID, Map<String, Object>> 이런식으로 저장되어있고 JSESSIONID에 해당하는 암호화된 코드 클라이언트로 보내고 클라이언트가 요청마다 이 코드를 보내면 서버는 키값에 해당하는 Map<String,Object>를 찾아서 우리가 저장해논 값을 출력하는거임

* HttpServletRequest에 getSession(true)면 이미 세션존재하면 있는 세션 리턴하고 없으면 생성함 false면 만들지않음 없으면 null값 리턴

* session.setAttribute(String,Object)로 세션에 넣기가능 session.getAttribute(String)으로 저장한 값 꺼내기 가능 없으면 null값 리턴함

* 세션은 클라이언트별로 하나만 생성이됨

* server.servlet.session.timeout=60 을 application.properties에 설정해서 세션 종료시간 설정가능 이것은 최근 세션요청으로부터 60초를 의미한다

* 위와 달리 아예 생성시점부터 세션 종료하게 하는 메서드도 있음 찾아봐라

### 2.로그인 기능

* 세션을 이용해서 구현하긴 했는데 기초적인 부분으로 구현한 느낌이 크다 더 관련 공부해서 좋게 만들어야됨

* 만드는 로직에서 LoginController 처럼 따로 컨트롤러 만드는 것이 좋아보임

* 홈 화면도 세션이 존재해서 해당 세션에 "login" 키 가 있으면 해당 value를 모델에 담아서 뷰로 넘기도록 만듬


## 정리

* 쿠키의 취약점과 이를 해결하기 위해서 세션이라는 걸 사용한다는 걸 알게됨

* 로그인을 대략적으로 어떤 느낌으로 구현하는지 알게 됨
